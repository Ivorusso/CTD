...
const inmobiliaria = {
  ...
  listarDepartamentos: function (array) {
    for (let i = 0; i < array.length; i++) {
      const dpto = array[i];
      const disponibilidad = dpto.disponible ? " disponible" : " alquilado";
      console.log(
        `id:${dpto.id}, precio \$${
          dpto.precioAlquiler
        }, esta${disponibilidad}, ambientes${
          dpto.cantidadHabitacion
        } ambientes , maximo ${dpto.cantidadPersonas} personas , ${
          dpto.aceptaMascotas ? "acepta mascotas" : "no mascotas"
        }, propietarios${dpto.propietarios}`
      );
    }
  },
  // C
...
// Ejecución aquí
const dptoPorId = inmobiliaria.buscarPorId(1);
inmobiliaria.listarDepartamentos([dptoPorId]);
console.log(o);
